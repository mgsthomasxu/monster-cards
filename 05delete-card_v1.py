...
while True:
    ...
    if choices == "Delete Card":
        ...
        if card_name in exist_cards:
            # Confirm Deletion
            confirm = easygui.ynbox(f"Are you sure you want to delete {card_name}",
                                     title="Confirm Deletion", choices=("Yes", "No"))
            if confirm:
                # Delete Card
                ...
                # Cancel Deletion
                easygui.msgbox(f"{card_name} deletion canceled.", "Canceled")
        else:
            # Card Not Found
